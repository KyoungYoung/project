...
const makeCards = (id, data) => {
  ...
  data.forEach(async (item, i) => {
    ...
    const res = await fetch(
      `${movie_detail_http}/${item.id}/watch/providers?` +
        new URLSearchParams({
          api_key: api_key,
        }) + kor);
    ...
    if (itemData.results.KR) {
      const ottWatch = itemData.results.KR.flatrate;
      const ottRent = itemData.results.KR.rent;
      const ottBuy = itemData.results.KR.buy;

      for (i in ottWatch) {
        let ottLogoStream = ottWatch[i].logo_path;

        ottImages += `<img class="" src="${original_img_url}${ottLogoStream}">`;
      }
    }
        ...
        <div class="movie" onclick="location.href = '/${item.id}'">
                <div class="movieImg-wrapper">
                    ...
                    <div class="hidden-title">
                        You Can Watch At
                        <div class="ottIcon">${ottImages}</div>
                    </div>
                    <div id="streamIn"></div>
                </div>
